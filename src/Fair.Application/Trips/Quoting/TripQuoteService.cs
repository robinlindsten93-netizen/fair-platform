...
{
    public TripQuote CreateQuote(Location pickup, Location dropoff, TransportMode mode, DateTimeOffset nowUtc)
    {
        var distanceMeters = EstimateDistanceMeters(pickup, dropoff);

        var speedMetersPerSecond = 35_000.0 / 3600.0; // ~35 km/h
        var durationSeconds = (int)Math.Ceiling(distanceMeters / speedMetersPerSecond) + 180;
        ...
        var expiresAt = nowUtc.AddMinutes(2);

        return TripQuote.Create(distanceMeters, durationSeconds, price, expiresAt, surgeMultiplier: null);
    }
